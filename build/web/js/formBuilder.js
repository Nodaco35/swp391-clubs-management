document.addEventListener("DOMContentLoaded", () => {
  ...
  if (!questionTemplateHtml || !optionTemplateHtml) {
    console.error("Lỗi: Không tìm thấy questionTemplate hoặc optionTemplate trong HTML.")
    return
  }

  console.log("formBuilder.js: DOMContentLoaded. Bắt đầu khởi tạo.")
  console.log(
      "formBuilder.js: Dữ liệu window.existingQuestions ban đầu:",
      JSON.parse(JSON.stringify(window.existingQuestions || [])),
  )

  if (window.existingQuestions && Array.isArray(window.existingQuestions) && window.existingQuestions.length > 0) {
    console.log(`formBuilder.js: Tìm thấy ${window.existingQuestions.length} câu hỏi đã có. Đang xử lý...`)
    if (questionsList) {
      while (questionsList.firstChild) {
        questionsList.removeChild(questionsList.firstChild)
      }
      console.log("formBuilder.js: Đã xóa các câu hỏi mặc định trong HTML (nếu có).")
    } else {
    ...
    window.existingQuestions.forEach((qData, index) => {
      console.log(`formBuilder.js: Đang tạo card cho câu hỏi #${index + 1}`, JSON.parse(JSON.stringify(qData)))
      try {
        ...
        console.error("formBuilder.js: Lỗi khi tạo/thêm card cho câu hỏi đã có:", JSON.parse(JSON.stringify(qData)), e)
      }
    })
    console.log("formBuilder.js: Hoàn thành xử lý câu hỏi đã có. Tổng số card đã thêm:", questionCount)
  } else {
    console.log("formBuilder.js: Không có câu hỏi đã có. Sử dụng câu hỏi mặc định từ HTML (nếu có).")
    if (questionsList) {
      const defaultQuestionCards = questionsList.querySelectorAll(".question-card")
      ...
      console.log(`formBuilder.js: Khởi tạo với ${questionCount} câu hỏi mặc định từ HTML.`)
    }
  }
  ...
  if (saveSuccessAlert && new URLSearchParams(window.location.search).has("success")) {
  ...
  if (saveErrorAlert && new URLSearchParams(window.location.search).has("error")) {
  ...
  if (formTitleInput)
    formTitleInput.addEventListener("input", () => {
      if (previewTitle) previewTitle.textContent = formTitleInput.value || "Đơn đăng ký"
    })
  if (formTypeSelect) {
  ...
  if (addInfoBtn) addInfoBtn.addEventListener("click", () => addNewQuestion("info"))
  if (saveBtn) saveBtn.addEventListener("click", () => submitForm("save"))
  if (publishBtn && publishDialog) publishBtn.addEventListener("click", () => publishDialog.classList.add("open"))
  ...
  tabItems.forEach((tab) => {
    tab.addEventListener("click", function () {
      ...
      activeTab = tabId
      if (tabId === "preview" && typeof window.updatePreview === "function") window.updatePreview()
    })
  })
  ...
  function checkQuestionLimit() {
    ...
    if (maxQuestionsAlert) maxQuestionsAlert.style.display = limitReached ? "flex" : "none"
  }
  ...
  function addNewQuestion(type = "text") {
    console.log("formBuilder.js: addNewQuestion, type:", type)
    ...
    if (!newQuestionCard) {
      console.error("Không thể tạo newQuestionCard từ template.")
      questionCount--
      updateQuestionCountDisplay()
      checkQuestionLimit()
      return
    ...
    newQuestionCard
        .querySelectorAll(".question-actions button, .question-type, .question-required")
  ...
  function createQuestionCard(qData, number) {
    ...
    console.log(
        `formBuilder.js: createQuestionCard - domId=${domId}, dbId=${qData.id}, number=${number}, type=${qData.type}, label=${qData.label}`,
    )
    console.log("formBuilder.js: createQuestionCard - qData.options:", qData.options)

    const html = questionTemplateHtml.replace(/{{id}}/g, domId).replace(/{{number}}/g, number)
    const tempDiv = document.createElement("div")
    tempDiv.innerHTML = html
    ...
    if (isFixedRequiredQuestion) {
      if (qData.label.includes("Chọn ban")) questionCard.dataset.requiredType = "department";
    ...
    console.log(`formBuilder.js: createQuestionCard - Xử lý options cho type: ${qData.type}`)
    if (qData.type === "info") {
      if (qData.options && qData.options !== "null") {
        try {
          ...
          if (contentTextarea && infoData.content) contentTextarea.value = infoData.content
          if (infoData.image) {
          ...
          console.error("formBuilder.js: Lỗi parse JSON options cho INFO:", qData.options, e)
          const contentTextarea = questionCard.querySelector(".question-content")
          if (contentTextarea && typeof qData.options === "string") contentTextarea.value = qData.options
        }
      }
    } else if (["radio", "checkbox", "select"].includes(qData.type)) {
      if (qData.options && qData.options !== "null" && typeof qData.options === "string") {
        console.log(`formBuilder.js: Options cho ${qData.type} (chuỗi từ JSP): '${qData.options}'`)
        const optionValues = qData.options.split(";;")
        console.log(`formBuilder.js: Options đã split cho ${qData.type}:`, optionValues)
        ...
        console.log(`formBuilder.js: Không có options hoặc options không phải chuỗi cho ${qData.type}:`, qData.options)
      }
    }
  ...
  function toggleQuestionFields(questionCard, type) {
    console.log(`formBuilder.js: toggleQuestionFields - cardId: ${questionCard.dataset.id}, type: ${type}`)
    ...
    if (type === "info" && infoGroup) {
      ...
      const isNewQuestion =
          ...
          (questionCard.dataset.id.startsWith("qNew") || questionCard.dataset.id.startsWith("qDup"))
      // Chỉ thêm option mặc định nếu là câu hỏi mới VÀ chưa có option nào (trường hợp load từ DB đã có options thì không thêm)
      if (optionsList && optionsList.children.length === 0 && isNewQuestion && !questionCard.dataset.originalId) {
        addOption(questionCard, "Tùy chọn 1")
      }
    } else if (placeholderGroup && !["info", "radio", "checkbox", "select"].includes(type)) {
  ...
  function addQuestionEventListeners(questionCard) {
    ...
    if (isHardcodedRequiredQuestion && !requiredType && labelInput && typeSelect) {
      if (labelInput.value.includes("Chọn ban") && typeSelect.value === "radio") {
    ...
    const isDepartmentQuestion = questionCard.dataset.requiredType === "department" || 
    ...
    if (moveUpBtn)
      moveUpBtn.addEventListener("click", () => {
        ...
        if (prev && (!isHardcodedRequiredQuestion || !prev.classList.contains("required-question"))) {
    ...
    if (moveDownBtn)
      ...
      })    // SỬA LỖI CÚ PHÁP: Xóa dấu \ thừa    if (deleteBtn)
    ...
    if (duplicateBtn) {
      duplicateBtn.addEventListener("click", () => {
        ...
        clone
            .querySelectorAll(".question-actions button, .question-type, .question-required")
            .forEach((el) => (el.disabled = false))
        const clonedRequiredCheckbox = clone.querySelector(".question-required")
        if (clonedRequiredCheckbox) {
          ...
          if (labelForCloned) labelForCloned.setAttribute("for", `required-${newId}`)
        }
        if (clone.dataset.imageData) {
          /* Giữ imageData hoặc xử lý lại */
        }
  ...
  function setupImageUpload(questionCard) {
    const imageInput = questionCard.querySelector(".question-image-file"),
        ...
        imagePreviewContainer = questionCard.querySelector(".image-preview-container"),
    ...
    if (!imageInput || !imageUploadArea || !imagePreviewContainer || !imagePreview || !uploadPlaceholder) return
  ...
  function addOption(questionCard, value = "") {
    ...
    const optionId = "opt" + Date.now() + "-" + optionCount + Math.random().toString(16).slice(2)
    console.log(
        `formBuilder.js: addOption - card: ${questionCard.dataset.id}, value: '${value}', newOptionId: ${optionId}`,
    )
    ...
    if (deleteBtn) {
      deleteBtn.addEventListener("click", (e) => {
        ...
        if (optionsList.children.length <= 1 && (questionType === "radio" || questionType === "select")) {
  ...
  window.updatePreview = function updatePreview() {
    ...
    questions.forEach((q) => {
      ...
      legend.innerHTML = q.label + (q.required ? ' <span style="color: red;">*</span>' : "")
      ...
      case "info":
        ...
        if (q.options) {
          try {
            ...
            console.error("formBuilder.js: Lỗi parse JSON options cho INFO preview:", q.options, e)
  ...
  function protectRequiredQuestions() {
    ...
    allCards.forEach(card => {
      ...
      if (isDepartmentQuestion) {
        // Nếu là form member, bảo vệ câu hỏi
        if (formType === "member") {
    ...
    console.log("formBuilder.js: Đã cập nhật bảo vệ câu hỏi theo loại form: " + formType);
  }
