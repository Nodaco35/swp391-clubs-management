...
function loadRatingData() {
  ...
  const eventId = document
    .querySelector('meta[name="eventId"]')
      ...
      plugins: {
        ...
        tooltip: {
          callbacks: {
            label: function (context) {
              ...
              const stars = 5 - context.dataIndex;
              return `${stars} sao: ${value} đánh giá (${percentage}%)`;
            },
          },
        },
        onClick: function (event, elements) {
          if (elements.length > 0) {
            ...
            const stars = 5 - index; // Từ index (0-4) sang sao (5-1)
            window.showUsersWithRating(stars); // Gọi hàm từ viewFeedback.js
          }
        },
      },
    },
...
function showNoDataMessage(chartElement) {
  ...
  message.innerHTML =
    '<i class="fas fa-info-circle"></i><br>Chưa có dữ liệu đánh giá';
