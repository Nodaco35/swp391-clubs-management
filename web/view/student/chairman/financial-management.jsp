        ...
        <div class="stats-section">
            ...
            <div class="stats-grid">
                ...
                <div class="stat-card">
                    ...
                    <div class="stat-content">
                        <h3><fmt:formatNumber value="${totalIncome - totalExpense}" type="currency" currencySymbol="₫"/></h3>
                        <p>Số dư hiện tại</p>
                    </div>
                </div>
                <div class="stat-card">
            ...
            <div class="section-header">
                ...
                <div class="section-actions">
                    <form action="${pageContext.request.contextPath}/chairman-page/financial-management" method="get" class="search-container">
                        <i class="fas fa-search search-icon"></i>
                        <input type="text" name="search" class="search-input" placeholder="Tìm kiếm giao dịch..." value="${param.search}">
                    </form>
                    <select class="filter-select" name="type" onchange="this.form.submit()" form="filterForm">
                        ...
                        <option value="expense" ${param.type == 'expense' ? 'selected' : ''}>Chi phí</option>
                    </select>
                    <select class="filter-select" name="termID" onchange="this.form.submit()" form="filterForm">
                        <option value="all" ${param.termID == 'all' || empty param.termID ? 'selected' : ''}>Tất cả kỳ</option>
                        <c:forEach var="term" items="${termIDs}">
                            <option value="${term}" ${param.termID == term ? 'selected' : ''}>${term}</option>
                        </c:forEach>
                    </select>
                    <form id="filterForm" action="${pageContext.request.contextPath}/chairman-page/financial-management" method="get" style="display: none;">
                ...
                <table class="events-table">
                    ...
                    <tbody>
                            ...
                            <tr>
                                <td>
                                    <c:choose>
                                        ...
                                        <c:otherwise>
                                            <span class="status COMPLETED">Chi phí</span>
                                        </c:otherwise>
                                    </c:choose>
                                </td>
                                ...
                                <td>
                                    <c:if test="${transaction.status == 'Pending'}">
                                        <a href="${pageContext.request.contextPath}/chairman-page/approve-transaction?transactionID=${transaction.transactionID}&action=approve" class="btn-action edit">
                                            <i class="fas fa-check"></i> Phê duyệt
                                        </a>
                                        <a href="${pageContext.request.contextPath}/chairman-page/approve-transaction?transactionID=${transaction.transactionID}&action=reject" class="btn-action assign">
                        ...
                        <c:if test="${empty transactions}">
                            <tr>
                                <td colspan="7" class="no-events">Không có giao dịch nào</td>
                            </tr>
                        </c:if>
                    </tbody>
                </table>
            ...
            <c:if test="${totalPages > 1}">
                <div class="pagination" style="padding: 20px; text-align: center;">
                    <c:if test="${currentPage > 1}">
                        <a href="${pageContext.request.contextPath}/chairman-page/financial-management?page=${currentPage - 1}&search=${param.search}&type=${param.type}&termID=${param.termID}" class="btn-secondary">
