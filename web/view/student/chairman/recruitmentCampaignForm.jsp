...
<body style="background-color: #d8d7ce;">
    ...
    <div class="container">
        ...
        <form id="recruitmentForm" data-mode="${empty campaign.recruitmentID || campaign.recruitmentID == 0 ? 'create' : 'edit'}" class="mb-6">
                ...
                <div class="card">
                    ...
                    <div class="form-grid">
                        <div>
                            ...
                            <div class="form-group">
                                <label for="templateId" class="form-label">Form đăng ký <span class="required">*</span></label>
                                <c:if test="${not empty formWarning}">
                                    <div class="alert alert-warning" style="margin-bottom: 10px; padding: 5px; color: #856404; background-color: #fff3cd; border: 1px solid #ffeeba; border-radius: 4px;">
                                        ${formWarning}
                                    </div>
                                </c:if>
                                <select id="templateId" name="templateId" class="form-control" required>
                                    ...
                                    <c:forEach items="${publishedTemplates}" var="template">
                                        <option value="${template.templateId}" ${template.templateId == campaign.templateID ? 'selected' : ''}>
                                            ${template.title} 
                                            <c:if test="${template.clubId != param.clubId && template.clubId != campaign.clubID}">
                        ...
                        <div>
                            <div class="form-group">
                                <label for="title" class="form-label">Tiêu đề hoạt động <span class="required">*</span></label>
                            ...
                            <div class="form-group">
                                ...
                                <textarea id="description" name="description" rows="4" 
                                          class="form-control" required>${campaign.description}</textarea>
                    ...
                    <div class="form-grid">
                        ...
                        <div class="form-group">
                            <label for="endDate" class="form-label">Ngày kết thúc <span class="required">*</span></label>
                ...
                <div class="card">
                    ...
                    <div class="stage-card interview">
                        ...
                        <div class="form-group">
                            ...
                            <select name="interviewLocationId" id="interviewLocationId" 
                                    class="form-control">
                                <option value="">-- Chọn địa điểm --</option>
                                <c:forEach items="${locations}" var="location">

                                    <option value="${location.locationID}">${location.locationName} - ${location.typeLocation}</option>
                                </c:forEach>
                            </select>
                        </div>
                    </div>
    ...
    <script>
        // Khởi tạo FormSaver khi DOM đã sẵn sàng
        document.addEventListener('DOMContentLoaded', function() {
            ...
            if (hasStoredData && document.getElementById('recruitmentForm').dataset.mode !== 'edit') {
                formSaver.showRestorePrompt(
                    'Phát hiện dữ liệu bạn đã điền trước đó nhưng chưa lưu. Bạn có muốn khôi phục không?',
                    function() { formSaver.restoreFormData(); }, // Khôi phục dữ liệu
                    function() { formSaver.clearSavedData(); }   // Xóa dữ liệu đã lưu
                );
            }
            
            // Xóa dữ liệu đã lưu khi form được submit thành công
            document.getElementById('recruitmentForm').addEventListener('submit', function() {
