            ...
            <div class="filter-search-container">
                <div>
                    <form action="${pageContext.request.contextPath}/financial/transaction-history" method="get">
                        ...
                        <label for="termFilter">Lọc theo kỳ:</label>
                        <select id="termFilter" name="termID" onchange="this.form.submit()">
                            <option value="" ${param.termID == '' ? 'selected' : ''}>Tất cả</option>
                            <c:forEach var="term" items="${terms}">
                                <option value="${term.termID}" ${param.termID == term.termID ? 'selected' : ''}>${term.termName}</option>
                            </c:forEach>
                        </select>
                        <label for="clubFilter">Lọc theo câu lạc bộ:</label>
                        <select id="clubFilter" name="clubID" onchange="this.form.submit()">
                            <option value="" ${param.clubID == '' ? 'selected' : ''}>Tất cả</option>
                            <c:forEach var="club" items="${clubs}">
                                <option value="${club.clubID}" ${param.clubID == club.clubID ? 'selected' : ''}>${club.clubName}</option>
                            </c:forEach>
                        </select>
                    </form>
                </div>
            ...
            <nav aria-label="Page navigation">
                <ul class="pagination">
                    <li class="page-item ${currentPage == 1 ? 'disabled' : ''}">
                        <a class="page-link" href="${pageContext.request.contextPath}/financial/transaction-history?page=${currentPage - 1}&transResult=${param.transResult}&termID=${param.termID}&clubID=${param.clubID}">Trước</a>
                    </li>
                    <c:forEach begin="1" end="${totalPages}" var="i">
                        <li class="page-item ${currentPage == i ? 'active' : ''}">
                            <a class="page-link" href="${pageContext.request.contextPath}/financial/transaction-history?page=${i}&transResult=${param.transResult}&termID=${param.termID}&clubID=${param.clubID}">${i}</a>
                        </li>
                    </c:forEach>
                    <li class="page-item ${currentPage == totalPages ? 'disabled' : ''}">
                        <a class="page-link" href="${pageContext.request.contextPath}/financial/transaction-history?page=${currentPage + 1}&transResult=${param.transResult}&termID=${param.termID}&clubID=${param.clubID}">Sau</a>
                    </li>
                </ul>
            </nav>
        </div>
