            ...
            <div class="flex-grow-1 p-4">
                <div class="container mt-4">
                    ...
                    <c:if test="${not empty approvalHistory}">
                        <h4 class="mt-4">Lịch sử duyệt đơn</h4>
                        <table class="table table-bordered mt-3">
                            ...
                            <tbody>
                                <c:forEach var="record" items="${approvalHistory}">
                                    <tr>
                                        <td><fmt:formatDate value="${record.actionAt}" pattern="dd/MM/yyyy HH:mm:ss" /></td>
                                        <td>
                                            <c:choose>
                                                <c:when test="${record.actionType == 'Approved'}">
                                                    <span class="badge bg-success">Đồng ý</span>
                                                </c:when>
                                                <c:when test="${record.actionType == 'Rejected'}">
                                                    <span class="badge bg-danger">Từ chối</span>
                                                </c:when>
                                                <c:otherwise>
                                                    <span class="badge bg-primary">Đăng ký tạo</span>
                                                </c:otherwise>
                                            </c:choose>
                                        </td>
                                        <td>${empty record.reason ? "-" : record.reason}</td>
                    ...
                    <c:if test="${empty approvalHistory}">
                        <div class="alert alert-info mt-3">Chưa có lịch sử duyệt cho CLB này.</div>
                    </c:if>
                </div>
