...
('Giải Bóng Đá Sinh Viên FPTU 2025', 'images/events/football2025.jpg', '...', '2025-06-05 08:00:00', '2025-06-05 10:00:00', 5, 1, TRUE, 120, 'PENDING', 'SU25'),
...
INSERT INTO TaskAssignmentDepartment
  (EventID, Term, TermStart, TermEnd, Description, TaskName, Status, StartedDate, DueDate)
VALUES
  (1, 'Trước sự kiện', '2025-06-01', '2025-06-05',
   'Chuẩn bị nội dung sự kiện', 'Chuẩn bị slide', 'ToDo',
...
INSERT INTO TaskAssignmentDepartmentResponser
  (ClubDepartmentID, TaskAssignmentDepartmentID)
VALUES
  (1, 1);

CREATE TABLE TaskAssignmentMember (
    TaskAssignmentMemberID INT PRIMARY KEY AUTO_INCREMENT,
    ...
    StartedDate DATE,
    DueDate DATETIME,
    FOREIGN KEY (TaskAssignmentDepartmentResponserID)
        REFERENCES TaskAssignmentDepartmentResponser(TaskAssignmentDepartmentResponserID)
        ON DELETE CASCADE
) CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci;
INSERT INTO TaskAssignmentMember
  (TaskAssignmentDepartmentResponserID, Description, TaskName, Status, StartedDate, DueDate)
VALUES
  (1, 'Soạn thảo slide giới thiệu', 'Soạn slide', 'Processing', '2025-06-02', '2025-06-04'),
  (1, 'Thiết kế hình ảnh minh họa', 'Thiết kế ảnh',    'ToDo',      '2025-06-03', '2025-06-06');

-- Bảng này chứa các thành viên của Ban đó được giao nhiệm vụ
CREATE TABLE TaskAssignmentMemberResponser (
    TaskAssignmentMemberResponserID INT PRIMARY KEY AUTO_INCREMENT,
    TaskAssignmentMemberID INT NOT NULL,
    MemberID VARCHAR(10) NOT NULL,
    FOREIGN KEY (TaskAssignmentMemberID)
        REFERENCES TaskAssignmentMember(TaskAssignmentMemberID)
        ON DELETE CASCADE,
...
-- Table for tracking task progress
CREATE TABLE TaskProgress (
    TaskProgressID INT PRIMARY KEY AUTO_INCREMENT,
    TaskAssignmentMemberID INT NOT NULL,
    MemberID VARCHAR(10) NOT NULL,
...
INSERT INTO TaskProgress
  (TaskAssignmentMemberID, MemberID, Progress, Notes)
VALUES
  (1, 'U001', 25, 'Bắt đầu làm slide'),
  (1, 'U001', 50, 'Hoàn thành 50% slide'),
  (2, 'U002', 75, 'Hoàn thành 75% ảnh');


-- Table for task feedback
CREATE TABLE TaskFeedback (
    TaskFeedbackID INT PRIMARY KEY AUTO_INCREMENT,
    TaskAssignmentMemberID INT NOT NULL,
    ReviewerMemberID VARCHAR(10) NOT NULL,
    Rating ENUM('Positive', 'Neutral', 'Negative') DEFAULT 'Neutral',
    Comment TEXT,
    ProgressAdjustment INT, -- Can be positive or negative adjustment to progress
    CreatedAt TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
    FOREIGN KEY (TaskAssignmentMemberID)
        REFERENCES TaskAssignmentMember(TaskAssignmentMemberID)
        ON DELETE CASCADE,
    FOREIGN KEY (ReviewerMemberID)
        REFERENCES Users(UserID)
        ON DELETE CASCADE
) CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci;
...
CREATE TABLE TaskComment (
    TaskCommentID INT PRIMARY KEY AUTO_INCREMENT,
    TaskID INT NOT NULL, -- This can refer to either TaskAssignmentDepartment or TaskAssignmentMember
    UserID VARCHAR(10) NOT NULL,
    Comment TEXT NOT NULL,
    CreatedAt TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
    IsTaskAssignmentDepartment BOOLEAN DEFAULT FALSE, -- TRUE if TaskID refers to TaskAssignmentDepartment, FALSE if it refers to TaskAssignmentMember
    FOREIGN KEY (UserID)
        REFERENCES Users(UserID)
        ON DELETE CASCADE
) CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci;
...
-- Insert sample data
INSERT INTO TaskProgress (TaskAssignmentMemberID, MemberID, Progress, Notes) VALUES
(1, 'U001', 25, 'Started working on this task'),
(1, 'U001', 50, 'Halfway done with the implementation'),
(2, 'U002', 75, 'Almost completed, need to review');

INSERT INTO TaskFeedback (TaskAssignmentMemberID, ReviewerMemberID, Rating, Comment, ProgressAdjustment) VALUES
(1, 'U007', 'Positive', 'Good work so far, keep it up!', 5),
(2, 'U007', 'Neutral', 'On track but need to improve documentation', 0);

INSERT INTO TaskComment (TaskID, UserID, Comment, IsTaskAssignmentDepartment) VALUES
(1, 'U001', 'Đã bắt đầu làm việc trên nhiệm vụ này', FALSE),
(1, 'U007', 'Hãy cập nhật tiến độ thường xuyên nhé', FALSE),
(1, 'U008', 'Mình sẽ hỗ trợ nếu cần', TRUE);
...
CREATE TABLE TaskProgress (
    ...
    Notes TEXT,
    CreatedAt TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
    FOREIGN KEY (TaskAssignmentMemberID) REFERENCES TaskAssignmentMember(TaskAssignmentMemberID) ON DELETE CASCADE,
...
CREATE TABLE TaskFeedback (
    ...
    CreatedAt TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
    FOREIGN KEY (TaskAssignmentMemberID) REFERENCES TaskAssignmentMember(TaskAssignmentMemberID) ON DELETE CASCADE,
    FOREIGN KEY (ReviewerMemberID) REFERENCES Users(UserID) ON DELETE CASCADE
);
