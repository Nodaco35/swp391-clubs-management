...
public class ClubDepartmentDAO {
    ...
    public ClubDepartment getDepartmentById(int clubDepartmentId) {
      ...
      /**
     * Get a ClubDepartment object by its DepartmentID
     * @param departmentId The ID of the department
     * @return ClubDepartment object, or null if not found
     */
    public ClubDepartment getDepartmentByDepartmentID(int departmentId) {
        ClubDepartment department = null;
        Connection conn = null;
        PreparedStatement stmt = null;
        ResultSet rs = null;
      ...
      /**
     * Get all departments for a specific user in a club
     * @param userId The ID of the user (String type)
     * @param clubId The ID of the club
     * @return List of ClubDepartment objects where the user is a member
     */
    public List<ClubDepartment> getDepartmentsByUserAndClub(String userId, int clubId) {
        List<ClubDepartment> departments = new ArrayList<>();
        Connection conn = null;
        PreparedStatement stmt = null;
        ResultSet rs = null;
        ...
        try {
            ...
            String sql = "SELECT cd.ClubDepartmentID, d.DepartmentID, d.DepartmentName, d.DepartmentStatus, d.Description, cd.ClubID " +
                         "FROM ClubDepartments cd " +
                         "JOIN Departments d ON cd.DepartmentID = d.DepartmentID " +
                         "JOIN UserClubs uc ON cd.ClubDepartmentID = uc.ClubDepartmentID " +
                         "WHERE uc.UserID = ? AND cd.ClubID = ? " +
                         "ORDER BY d.DepartmentName";
            stmt = conn.prepareStatement(sql);
            ...
            System.out.println("Error fetching user's departments in club: " + e.getMessage());
        } finally {
            try {
                if (rs != null) rs.close();
                if (stmt != null) stmt.close();
