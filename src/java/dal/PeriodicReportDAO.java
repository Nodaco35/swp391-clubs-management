...
public class PeriodicReportDAO extends DBContext {
    ...
    public boolean deleteUserToActiveClub(String userId, int clubId) {
        String sql = """
        delete from ActivedMemberClubs where UserID = ? and ClubID = ? and TermID = (SELECT TermID FROM semesters WHERE Status = 'ACTIVE' LIMIT 1);
        """;
